...
Le code se constitue de 5 fichier principaux :
    ...
    - rpr.py :  RPR qui une classe héritant de Robot_Arm
                C'est la classe du robot étudié dans le BE où se trouvent tous les calculs de modélisation.
                Si on voulait modéliser un autre robot il faudrait créer une autre classe du type RPR qui extend Robot_arm et qui surcharge toutes les méthodes données dans la classe Robot_Arm.
    ...
    -movement_law.py : contient la fonction loi de mouvement. Si on voulait changer de loi de mouvement il faudrait que la fonction ai les mêmes paramètres et renvoient des vecteurs de même forme
